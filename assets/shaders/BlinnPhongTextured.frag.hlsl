...
float4 main(PSInput input) : SV_TARGET {
    ...
    for(int i = 0; i < num_point_lights; i++) {
        result += CalcPointLight(pointLights[i], input.worldNormal, input.worldPos, calcSpecular, CalcBlinnPhongSpecular(normalize(pointLights[i].position.xyz - input.worldPos), input.worldNormal, normalize(viewPos - input.worldPos), shininess));
    }
    for(int i = 0; i < num_dir_lights; i++) {
       ...
       result += CalcDirectionalLight(light, input.worldNormal, calcSpecular, CalcBlinnPhongSpecular(normalize(-directionalLights[i].direction.xyz), input.worldNormal, normalize(viewPos - input.worldPos), shininess));
    }
