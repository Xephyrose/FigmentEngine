...
float4 main(PSInput input) : SV_TARGET {
    ...
    for(int i = 0; i < num_point_lights; i++) {
        ...
        specular += CalcPointLightSpecular(pointLights[i], input.worldNormal, calcSpecular, CalcBlinnPhongSpecular(normalize(-directionalLights[i].direction.xyz), input.worldNormal, normalize(viewPos - input.worldPos), shininess));
    }
    for(int i = 0; i < num_dir_lights; i++) {
        ...
        specular += CalcDirectionalLightSpecular(light, calcSpecular, CalcBlinnPhongSpecular(normalize(-directionalLights[i].direction.xyz), input.worldNormal, normalize(viewPos - input.worldPos), shininess));
    }
    for(int i = 0; i < num_spot_lights; i++) {
        diffuse += CalcSpotLightDiffuse(spotLights[i], input.worldNormal, input.worldPos);
        specular += CalcSpotLightSpecular(spotLights[i], input.worldPos, calcSpecular, CalcBlinnPhongSpecular(normalize(-directionalLights[i].direction.xyz), input.worldNormal, normalize(viewPos - input.worldPos), shininess));
    }

    float3 lighting = (calcAmbient + diffuse) * calcAlbedo + specular;
