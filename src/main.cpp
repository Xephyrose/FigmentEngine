...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[])
{
    ...
    *appstate = appState;
    ...
    constexpr SDL_GPUShaderFormat formatFlags = SDL_GPU_SHADERFORMAT_SPIRV | SDL_GPU_SHADERFORMAT_MSL;
...
SDL_AppResult RenderFrame(AppState* appState) {
    if (appState->debug) {
        ...
        ImGui::Image(static_cast<ImTextureID>(reinterpret_cast<intptr_t>(appState->shadowMap)), ImVec2(256.0f, 256.0f)); // eventually turn this into the shadow map atlas
