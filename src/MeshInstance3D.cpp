...
void MeshInstance3D::Draw(SDL_GPUCommandBuffer *commandBuffer) {
...
void MeshInstance3D::DrawShadow(SDL_GPUCommandBuffer *commandBuffer, SDL_GPURenderPass* renderPass) {
    AppState* appState = &AppState::Get();
    const Mesh* _mesh = appState->GetMesh(this->mesh);
    if (!_mesh || !_mesh->isOnGPU) return;
